...
{% block base_stylesheets %}
  {{ parent() }}
  <link rel="stylesheet" href="{{ asset('css/overwrites') }}" />
{% endblock %}
...
{% block base_body %}
  <h1>Hello World! I am overwrited!</h1>
{% endblock %}
...
Alice provides some Twig functions that might be helpful and super easy to use. You can get a complete overview here: https://design.stroeermediabrands.de/twig-functions/


## Alice features

Alice has some useful features, which can be used by the assets too.

### bootstrap

Alice is using [bootstrap](https://getbootstrap.com/docs/3.3/) (currently version 3.3.7). By integrating Alice in our assets, the assets could use the embedded bootstrap version with all its functionalities too.

### smb-tracking

Alice uses our centralized tracking module, smb-tracking. We could make use of it too. You can find the smb-tracking documentation [here](https://github.com/smb-ag/smb-tracking).

### smb-element-observer

The smb-element-observer is also implemented and could be used by the assets too. The smb-element-observer is documented [here](https://github.com/smb-ag/smb-element-observer). The module is listed as ``smb-element-observer`` in the requireJS config.

### lazyloading

All images are lazyloaded. We are using this [npm package](https://www.npmjs.com/package/vanilla-lazyload) to enable this feature. By integrating Alice in our assets, we could use this for all our images, video and iframes on our assets too. Just add the HTML class ``lazyload`` to the element you want to be lazyloaded. The module is listed as ``vendor/lazyload`` in the requireJS config.

## Requirements

In order to use Alice, these requirements are needed on the assets
